...
class ContainerSelectionTests: CanvasAfterDidLoadTests {
    ...
    func test_2elementsAreWrappedInContainer_whenClickBetweenElements_shouldSelectContainer() async throws {
        ...
        _ = try await selectContainer(at: .coord(80))
    }
    ...
    func drawElement(from: CGPoint, to: CGPoint) throws -> A11yDescription {
        try XCTUnwrap(drawRect(from: from, to: to)?.model as? A11yDescription)
    }
    
    func selectElement(at coordinate: CGPoint) async throws -> A11yDescription {
        click(coordinate)
        let selected = try await awaitSelected()
        let element = try XCTUnwrap(selected as? A11yDescription)
        return element
    }
    
    func selectContainer(at coordinate: CGPoint) async throws -> A11yContainer {
        click(coordinate)
        let selected = try await awaitSelected()
        let container = try XCTUnwrap(selected as? A11yContainer)
        return container
    }
}
